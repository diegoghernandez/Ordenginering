...
LOCALES.forEach((locale) => {
	const t = getJSON('../assets/i18n/pages/Customer.json')[locale]

	test.describe(`${locale}: Customer page e2e tests`, () => {
		...
		test('Should render correctly', async ({ page }) => {
			...
			const {
				...
				passwordFormTranslation,
				emailFormTranslation,
			} = t
			...
			await expect(
				profileArticle.getByLabel(profileFormTranslation.labels.birthDate)
			).toBeVisible()
			await expect(
				profileArticle.getByRole('button', {
					name: profileFormTranslation.submitLabel,
				})
			).toBeVisible()
			...
			await expect(
				passwordArticle.getByLabel(
					passwordFormTranslation.labels.newPassword
				)
			).toBeVisible()
			await expect(
				passwordArticle.getByLabel(
					passwordFormTranslation.labels.repeatPassword
				)
			).toBeVisible()
			await expect(
				passwordArticle.getByRole('button', {
					name: passwordFormTranslation.submitLabel,
				})
			).toBeVisible()
			...
			await expect(
				emailArticle.getByLabel(emailFormTranslation.labels.newEmail)
			).toBeVisible()
			await expect(
				emailArticle.getByLabel(emailFormTranslation.labels.currentPassword)
			).toBeVisible()
			await expect(
				emailArticle.getByRole('button', {
					name: emailFormTranslation.submitLabel,
				})
			).toBeVisible()
		})
		...
		test.describe('Profile form tests', () => {
			...
			test('Should full the formulary and get a error message', async ({
				...
				await profileArticle
					...
					.fill('New name')
				await profileArticle
					.getByLabel(profileFormTranslation.labels.birthDate)
					.fill('2020-02-02')
				...
				await expect(
					profileArticle.getByText('No older enough')
				).toBeVisible()
			})
			...
			test('Should full the formulary and change the profile information correctly', async ({
				...
				await expect(
					profileArticle.getByText('Change profile correctly')
				).toBeVisible()
			})
		})
		...
		test.describe('Password form tests', () => {
			...
			test('Should full the formulary and get a error message', async ({
				...
				await passwordArticle
					.getByLabel(passwordFormTranslation.labels.newPassword)
					.fill('1234')
				await passwordArticle
					.getByLabel(passwordFormTranslation.labels.repeatPassword)
					.fill('1234')
				...
				await expect(
					passwordArticle.getByText('Incorrect password')
				).toBeVisible()
			})
			...
			test('Should full the formulary and change the password correctly', async ({
				...
				await expect(
					passwordArticle.getByText('Change password correctly')
				).toBeVisible()
			})
		})
		...
		test.describe('Email form tests', () => {
			...
			test('Should full the formulary and get a error message', async ({
				...
				await currentPasswordLabel.fill('wrong')
				await emailArticle
					.getByLabel(emailFormTranslation.labels.newEmail)
					.fill('newemail@new.com')

				await emailArticle
					.getByRole('button', { name: emailFormTranslation.submitLabel })
					.click()

				await expect(emailArticle.getByText('Warning')).toBeVisible()
				await expect(
					emailArticle.getByText('Incorrect password')
				).toBeVisible()
			})
			...
			test('Should full the formulary and change the email correctly', async ({
				...
				await expect(
					emailArticle.getByText('Change email correctly')
				).toBeVisible()
			})
		})
	})
