...
LOCALES.forEach((locale) => {
   const t = getJSON('../i18n/pages/Orders.json')[locale]
   ...
   test.describe(`${locale}: Customer orders page tests`, () => {
      ...
      test('Should render correctly', async ({ page }) => {
         ...
         for (const element of await page.getByRole('article').all()) {
            ...
            await expect(element.getByText(new Intl.DateTimeFormat(locale, { dateStyle: 'medium' })
               .format(new Date(DATE)))).toBeVisible()
      ...
      test('Should show correctly one order when you click in one of the cards', async ({ page }) => {
         ...
         await expect(dialogElement.getByText(
            `${orderModalTranslation.date}: ${new Intl.DateTimeFormat(locale, { dateStyle: 'short', timeStyle: 'medium' })
            .format(new Date(DATE))}`)).toBeVisible()
