...
LOCALES.forEach((locale) => {
   const t = getJSON('../i18n/pages/Checkout.json')[locale]
   ...
   test.describe(`${locale}: Checkout page tests`, () => {
      ...
      test('Should render the checkout page correctly after login and get the cookie', async ({ page, context }) => {
         ...
         await expect(page.getByRole('button', { name: t.form.submitLabel })).toBeVisible()
         await expect(page.getByRole('link', { name: t.returnToChoose })).toBeVisible()
      })
      ...
      async ({ page, context, worker }) => {
         ...
         await expect(page.getByRole('link', { name: t.form.dialogAccept })).toBeVisible()
      })
   })
})