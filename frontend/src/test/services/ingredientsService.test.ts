import { IngredientTypes } from '@/lib/constants/ingredientTypes'
	...
	describe('saveIngredient', () => {
		...
		it('Should throw an error if the ingredient is repeated', async () => {
			...
			const imageFile = new Blob([
				await readFile('src/test/e2e/static/test.jpg'),
			])
			const ingredientFile = new Blob([bytes], { type: 'application/json' })
			...
			await expect(saveIngredient(data)).rejects.toThrow(
				'Repeat names are not allowed'
			)
		})
