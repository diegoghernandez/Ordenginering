...
export function ShowOrder({ currentLocal }: Props) {
   ...
   return (
      <section className={Styles['order-styles']}>
         <h2>Total: <strong>$<MotionNumber
            value={pizzaList.map((pizza) =>
         ...
         >
            {t.checkoutLink.name + ' '}
            <strong><MotionNumber value={pizzaList.map(({ quantity }) => quantity).reduce((acc, current) => acc + current, 0)}/></strong>
               ...
               pizza?.map((pizzaInOrder) => (
                  <CardContainer key={pizzaInOrder.idPizza} styleClass={String(Styles['card-separation'])}>
                     <>
                        ...
                        <p>$<MotionNumber
                              value={getPizzaPrice(pizzaInOrder.pizzaIngredients.length, pizzaInOrder.size, pizzaInOrder.quantity)}
                           />
                        </p>
