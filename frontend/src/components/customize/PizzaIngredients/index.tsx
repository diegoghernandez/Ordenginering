...
export function PizzaIngredients({
	...
	const removeIngredient = useDesireIngredients(
		...
		<>
			<div className={Styles['ingredient-buttons']}>
				{ingredientTypeList.map((type, index) => (
					...
					>
						{ingredientTypeListTranslate[index][0] +
							ingredientTypeListTranslate[index]
								.substring(1)
								.toLowerCase()}
					</button>
				))}
			...
			<div className={Styles['ingredients-container']}>
				{ingredientList
					.filter(
						({ ingredientType }) =>
							desiredType === ingredientTypeList[0] ||
							desiredType === ingredientType
					)
					.map((ingredient) => (
						<CardContainer
							key={ingredient.idIngredient}
							styleClass={Styles['ingredients-card']}
						>
							<>
								<ImgContainer
									locale={locale}
								...
								<SelectQuantity
									valueToShow={
										ingredients.length === 0
											? prebuildIngredientIDs?.includes(
											...
											: ingredients.filter(
													(element) =>
														element.id === ingredient.idIngredient
											  )[0]?.quantity ?? 0
									}
									minValue={0}
									maxValue={2}
