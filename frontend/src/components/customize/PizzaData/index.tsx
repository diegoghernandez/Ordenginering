...
interface Props {
	...
	localForModalLinks: LocalesString
	t: {
		...
		<CardContainer styleClass={Styles['customize-description']}>
			<>
				...
				<p>
					...
					{getPizzaPrice(
						ingredients.length === 0
							? prebuildIngredients.length
							: ingredients.reduce(
									(prev, now) => prev + now.quantity,
									0
							  ),
				...
				<IngredientsContainer
					ingredients={
						ingredients.length === 0
							? prebuildIngredients.map((ingredient) => ({
				...
				<AddCustomizePizza
					...
					pizza={{
						pizzaName: {
							en: 'Custom ' + pizza.pizzaName.en.replace('-', ' '),
							es:
								pizza.pizzaName.es.at(0)?.toUpperCase() +
								pizza.pizzaName.es.substring(1).replace('-', ' ') +
								' personalizada',
						},
