...
export function SignUpForm({ t }: Props) {
	const { isLoading, error, setError, response, handlePromise } =
		useServicePromise<CustomerDto, string>(registerCustomer)

	const labels = t.labels

	const handleData = (formValues: FormData) => {
		if (
			getFormValue(labels.password, formValues) !==
			getFormValue(labels.confirmPassword, formValues)
		) {
			setError({
